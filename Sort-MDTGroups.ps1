...
#Contributors:
# D.Murray
#Latest Update: 09/24/2022 12:01:21 D Murray
#Patch Notes:
#v1.0.2 - Added File Selection vs HardCoded File Name.
...
function Get-File {   
    ...
    $OpenFileDialog.Title = if($Title -eq $null){"Select File"}else{$Title}
    $OpenFileDialog.Multiselect=$true  #does not work
    $OpenFileDialog.filter = if($Filter -ne $null){$Filter}
...
$BackupFolderPath = "$((split-path $FileToEdit),"Backups" -join '\')\"  #v1.0.2
...
If ($BackupFileOn -eq $true){
    #Copy the original file to the specified folder and append a data value to its name. 
     If (Test-Path -Path $BackupFolderPath){
        #Copys the file
        Try{
            Copy-Item $FileToEdit `
                -Destination $BackupFolderPath\DriverGroups$((Get-Date).ToString('MM-dd-yyyy')).xml -Force -ErrorAction Stop}
